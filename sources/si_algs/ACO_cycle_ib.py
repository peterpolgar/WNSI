...
if __name__ == "__main__":
    ...
    routalgCost = rc_tenyezo * (sensorCount + 1) * 3
    ...
    feroBestArray = [[0] * (sensorCount + 1) for _ in range(sensorCount)]
            ...
            if sharedMem[0] == 0:
                    ...
                    if countkommn[dex] == kommN - 1:
                        for ll in range(sensorCount + 1):
                            if neighbors_dists[dex][ll] != -1 and (ll == sensorCount or battCapArray[ll] != -1):
                                zj = (1 - ferDecay) * feroArray[dex][ll] + feroBestArray[dex][ll]
                                if zj < ferMin:
                                    zj = ferMin
                                elif zj > ferMax:
                                    zj = ferMax
                                feroArray[dex][ll] = zj
                                feroBestArray[dex][ll] = 0
                ...
                if battCapArray[fad] > 0 and len(antBackQueues[fad]) > 0:
                    for que, tourLen in zip(antBackQueues[fad], antFeroBackQueues[fad]):
                        ...
                        if battCapArray[prevSens] > enoughEnergy:
                            ...
                            tbv = Qconst / tsco
                            if tbv > feroBestArray[fad][prevSens]:
                                feroBestArray[fad][prevSens] = tbv
                        else:
                            continue
                        # go back
                        if len(que) == 0:
                ...
                if wakeFulArray[dex] and countkommn[dex] % lepeskoz == 0:
                    ...
                    # if send to sink
                        ...
                        tbv = Qconst / neighbors_dists[dex][sensorCount]
                        if tbv > feroBestArray[dex][sensorCount]:
                            feroBestArray[dex][sensorCount] = tbv
                # forward ants
