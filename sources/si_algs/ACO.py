            ...
            if sharedMem[0] == 0:
                ...
                if battCapArray[fad] > 0 and len(antBackQueues[fad]) > 0:
                    for que, tourLen in zip(antBackQueues[fad], antFeroBackQueues[fad]):
                        ...
                        feroSumArray[fad][prevSens] += 1 / tsco
                        # go back
                        if len(que) == 0:
                            continue
                        antSendTo = que[-1]
                        numbytes = 18 + 2 * len(que) + 2
                ...
                if wakeFulArray[dex] and countkommn[dex] % lepeskoz == 0:
                    ...
                    # if send to sink
                        ...
                        feroSumArray[dex][sensorCount] += 1 / neighbors_dists[dex][sensorCount]
                # forward ants
                if len(antForwQueues[dex]) > 0:
                    for que in antForwQueues[dex]:                        
                        ...
                        # if send to sink
                            ...
                            feroSumArray[dex][sensorCount] += 1 / tsco
                            # go back
                            antSendTo = que[-1]
                            numbytes = 18 + 2 * len(que)
                            locRecCost = oneByteRecCost * numbytes
